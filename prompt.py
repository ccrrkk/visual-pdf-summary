...
## 简报结构要求
- **格式**:一个完整的、标准的Markdown文档. markdown文档请你以论文标题为题目,并在文档开头注明论文的作者和出处(例如会议名称和年份).在文档正文的第一行注明论文的基本信息
如果有的话,论文信息可以包含authors: "<作者列表>";\n affiliation: "<核心作者单位>";\n venue: "<发表会议或期刊名称,年份,具体的期和卷>";\nsource: "<论文来源,会议/期刊/arxiv链接/doi链接>.\n
以上基本信息注明一次即可,后续正文中不需要重复注明.**请勿**在正文中增加一个单独的"论文信息"章节来重复注明这些基本信息.

- **必须包含的章节**:
    - **简介**:使用**最简洁,最清晰**的语言概括论文的核心贡献和创新点,字数控制在200字以内.
    - **研究背景**:研究的背景,动机和核心思想.研究背景需要生动形象，引人入胜
    - **研究问题**:明确指出论文试图解决的核心问题是什么,这个问题为什么重要,以及目前在这个问题上存在的挑战和不足.注意要和研究背景衔接好,形成一个完整的逻辑链条.研究问题需要清晰明确,让读者能够理解这个问题的确切含义和重要性.
    - **研究内容**:写作逻辑：简要概括研究的核心假设或研究对象,解释研究者试图通过什么新视角或者新方法来解决上述背景中提到的问题。注意要和研究问题衔接好,形成一个完整的逻辑链条.研究内容需要清晰明确,让读者能够理解研究者的核心假设或者研究对象是什么,以及他们试图通过什么新视角或者新方法来解决上述背景中提到的问题.
    - **研究方法**:要求包括研究提出的核心方法,算法或流程描述,务必包含必要的说明图表,数学公式和推导.请确保你的说明清晰,详细,准确.**可以**对论文中描述不清或隐含的重要背景知识进行补充说明。
    - **研究结果**:实验及其设计思路;实验采用的方法,模型,数据集,评价指标等.**实验变量,实验设计请你不要有遗漏**;主要实验结果和必要的图表.这一部分可以适当简洁,只需呈现**最关键**的实验结论,图表和数据. 对于实验结果, 你要总结原作者对于结果和具体机制的分析和解释,并且你也可以在此基础上进行适当的补充分析和解释,但要确保你的补充分析和解释是基于论文内容的,而不是你自己的主观臆断.
...
请不要搞错插入的图片名称!特别注意: 你**不要**插入页面整体截图(如0.png),而是要插入被红色框标注的图片(如如Table_1_0.png和Fig_1_1.png等).
- **插入原则**:
    - **精准匹配**:确保标记中的文件名与pdf页面图片被中用红/蓝色框和红/蓝字标注的名称(%s)**完全一致**.注意你的名字**不要参考**原始标题的figure/table标注.请你仔细检查以确保**图片名称的正确性**,**不要出现拼写错误,格式错误或者虚构不存在的图片**.
    - **必要性**:在"方法"部分插入核心流程/算法/架构图;在"结果"部分插入最关键的数据图表和表格.避免冗余.
    - **完整性**:如果你在某个章节中引用了图片或表格,请**确保这些图片或表格都被你插入进入了正文中**,并且每张图片或表格都有相应的说明文字且与你正文中所指的能完成对应.注意**不要**出现引用了图片或者表格,但是报告中没有插入图片或者表格的情况.
    - **补充说明**:在每个插入的图表的说明框[]内,请附上简要的文字说明,解释该图表所展示的关键信息.
    - 建立正文和图片之间的清晰联系,确保读者能理解每个图表在支持论文论点中的作用. 示例: 
...
### 3. 语言
- 你的文档的主体部分必须使用**中文**, 但是:1.论文的标题以及论文基本信息保留英文 2. 论文中的一些专有名词且目前没有广泛采用的中文翻译名的,如token,transformer等,保留英文. 作者论文中提出来的新名词,如有必要,可以在第一次出现时加上中文翻译.
- 你在翻译论文中的一些重要术语时,请务必确保**相关学科背景**翻译的准确性和贴切性.请你在第一次出现这个术语的时候,在括号里注明一下这个术语的英文原文,以确保读者能够理解这个术语的确切含义. 
### 4. 内容真实性
- **必须严格基于论文内容**：简报的主体部分必须忠实反映论文明确陈述的内容。
- **严禁杜撰**：严禁编造论文中未出现的实验数据、结果、方法细节或结论。任何推断都必须明确其依据来源于论文的哪个部分。
### 5. 原始论文的数据来源
- 如果论文中提到的**关键数据集或者关键引用实验结果**是来自于公开的第三方数据集或者第三方实验结果,请你务必根据**原始论文的标注信息**在简报中注明这个数据集或者实验结果的来源.注意这一点仅限于论文中提到的**关键数据集或者实验结果**,对于一些次要的数据集或者引用实验结果你可以选择不注明来源,但要确保不影响对论文核心内容的理解.
